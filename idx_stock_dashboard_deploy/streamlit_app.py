...
# ─────────────────────────────
# 🔥 LIVE TICKER DATA (NEW - SAFE ADD)
# ─────────────────────────────
@st.cache_data(ttl=60)
def get_ticker_data():
    try:
        # IHSG
        ihsg = yf.download("^JKSE", period="1d", interval="1m", progress=False)

        if ihsg is None or ihsg.empty:
            return None, None, []

        ihsg_last = ihsg["Close"].iloc[-1]
        ihsg_prev = ihsg["Close"].iloc[0]
        ihsg_change = ((ihsg_last - ihsg_prev) / ihsg_prev) * 100

        # Top saham (aman, tidak ganggu logic utama)
        tickers = ["BBCA.JK", "BBRI.JK", "BMRI.JK", "TLKM.JK", "GOTO.JK"]
        ...
        return ihsg_last, ihsg_change, movers
...
# ─────────────────────────────
# 🔥 LIVE TICKER TAPE (NEW - SAFE ADD)
# ─────────────────────────────
ihsg_last, ihsg_change, movers = get_ticker_data()

def _ticker_color(val):
    if val is None:
        return "#94a3b8"
...
if isinstance(ihsg_last, (int, float)):
    ...
    <span style="margin-right:40px;">
        ...
        <span style="color:{_ticker_color(ihsg_change)};">
            {ihsg_change:+.2f}%
        </span>
    </span>
    """
